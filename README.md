...
- **v1.1.0**（2026-09-14）
  - 到点后继续计时显示超时（+Xs），食材名间距修复
  - 0 秒可见 + top100 食材补全 + 手法/超时分级/批次提醒/阶段提示
  - 红绿灯三档熟度系统（偏生/适中/偏熟），去掉选档 UI，改为计时中动态显示
  - 移除守锅（全屏）功能；提示文案折行修复，移除震动与通知，音量条移至声音后同行
  - 深度自查：修复 12 处 bug + 交互体验优化
  - CSV 数据管线 + 关键路径 E2E + CI/Pages；PWA 离线修复 + a11y 验收
  - 页脚显示版本号与构建日期；更新提示去掉冗余 toast（只保留底部可点击按钮）
- **v1.0.0**（2026-08-23）
  - Vite + TS 工程化重写，修复四个计时器 bug
  - 引擎单元测试基线（43 例）+ 修复 baseName 派生 bug
  - 初始静态版发布
